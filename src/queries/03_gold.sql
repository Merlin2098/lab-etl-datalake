...
-- Nota: source/year/month/day son particiones Hive-style catalogadas como
...
-- Consulta C: comparar dos fuentes del mismo esquema (ver docstring de
-- src/glue/transform.py sobre el patrón source=<name>/ en Bronze).
SELECT
    source,
    SUM(orders) AS orders,
    SUM(revenue) AS revenue
